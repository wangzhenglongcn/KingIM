...
public class FriendApplyController {
    ...
    public @ResponseBody PageInfo<FriendApply> getByFromUserId(int fromUserId, int status, int pageNum, int pageSize) {
        PageInfo<FriendApply> fa = friendApplyService.getByFromUserId(fromUserId, status, pageNum, pageSize);
        return fa;
    }

    // 查询我接收到的好友申请
    @RequestMapping(value = "getByToUserId")
    public @ResponseBody PageInfo<FriendApply> getByToUserId(int toUserId, int status, int pageNum, int pageSize) {
