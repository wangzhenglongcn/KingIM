...
//初始页面。获取分组和全部好友信息
$.post("http://114.251.65.250:2084/friendTypeController/getFriendType",{"userId":userId},function (res) {
   // console.log(res);
    let data = eval("("+res+")");
  //  console.log(data);
    let myType='';
    ...
    $("#typesData").val(map2json);
        $.each(data,function(k2,v2) {
            $.each(v2.friends, function (k1, v1) {
               // console.log(v1)
                myFriends += '<div class="col-md-4 col-sm-6">'
...
$(".typeReNameBtn").click(function(){
    ...
    if(typeName.trim()=="" || typeName.trim()==null){
        ...
        $.post("http://114.251.65.250:2084/friendTypeController/updateById",{"id":typeId,"newName":typeName},function(result) {
            if(result>0){
        ...
        $.post("/friendTypeController/getFriendsByTypeId",{"typeId":typeId},function (res) {
            ...
            $.each(data,function (k,v) {
                ...
                typeFriends += '<div class="col-md-4">'
                    ...
                    + '    <div class="myFriends_all ">'
                    + '    <span class="myFriends_block">' + v.nickName + '</span>'
